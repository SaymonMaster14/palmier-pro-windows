...
export interface ProbeResult {
  ...
  hasVideo: boolean; hasAudio: boolean; still?: boolean;
}
...
export async function probeMedia(path: string): Promise<ProbeResult> {
  ...
  if (!Number.isFinite(dur) || dur <= 0) { const stillCodecs = ["png", "mjpeg", "jpeg", "bmp", "webp", "tiff", "gif"]; const vcodec = String(v?.["codec_name"] ?? ""); if (v && !a && stillCodecs.includes(vcodec)) return { durationSec: 0, width: v?.["width"] as number | undefined, height: v?.["height"] as number | undefined, hasVideo: true, hasAudio: false, still: true }; throw new Error(`invalid/corrupt media (no duration): ${path}`); }
